...
contracts: list[SmartContract] = [
    SmartContract(path=import_contract(folder), name=folder.name, deploy=import_deploy_if_exists(folder))
...
def build(output_dir: Path, contract_path: Path) -> Path:
    ...
    build_result = subprocess.run(
        ["algokit", "--no-color", "compile", "python", str(contract_path.resolve()), f"--out-dir={output_dir}", "--output-source-map"],
    ...
    if app_spec_file_names:
        for file_name in app_spec_file_names:
            client_file = file_name
            generate_result = subprocess.run(
                ["algokit", "generate", "client", str(output_dir), "--output", str(_get_output_path(output_dir, deployment_extension))],
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=True,
            )
...
def main(action: str, contract_name: str | None = None) -> None:
    ...
    filtered_contracts = [c for c in contracts if contract_name is None or c.name == contract_name]
    match action:
        case "build":
